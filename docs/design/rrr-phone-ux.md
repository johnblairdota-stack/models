...
- During **DEBRIEF** the whole screen is replaced by a card reading **PHONES DOWN — TALK**, with only the reaction bar left live. This is risk **R3**'s mitigation and it belongs to the phone, not the TV.
  - ⚠️ **This disagrees with `rrr-social-round.md` §1's phase table and the table wins.** That table's `Phones` column reads **dark** for `RECAP`, `EXECUTION` and `VERDICT`, and **claims/nameplates only** for `DEBRIEF` — so what ships is the PHONES DOWN card on those three phases and the claim plate at DEBRIEF, which is where the talking actually happens and where a nameplate is the thing being argued about. The reason the card matters most at RECAP is bible **P2**: *"After an Expedition, crew phones go dark: no logs, no replay, no notes."* A claim box there hands the guide twenty idle seconds holding a **producible** alibi, composed while the evidence is still on screen, which attacks the one unwitnessed asset D13 leaves standing. The persistent ROLE tab (§2.3) stays live on all three, because it is hold-to-reveal and therefore a thing you check rather than a thing left on screen.
