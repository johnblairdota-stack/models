...
import { raggedBreak } from './surfaces/wallstages.js';
...
/**
 * 🕸️ **THE STRAIN TELL — crazing on wall that is about to come down.** '[amount, wall
 * threshold, darkness]'; 0 in x is inert and every existing damage-armed picture is bit-identical.
 *
 * 'damagefield.js' writes the A channel of 'tDamage' with how close each surviving cell is to
 * being brought down by 'support.js' collapse() — the SAME sweep, at a relaxed threshold, so
 * the crack is the rule one step early rather than a decoration that resembles it.
 *
 * 🎯 **THE CRACKS RUN ALONG THE PLATE WALLS BECAUSE THAT IS WHERE THE WALL WILL ACTUALLY BREAK.**
 * '_bgPlate' is a per-plate CONSTANT over a warped lattice (see uPlate) — the file's own model of
 * *plaster does not erode, it snaps in plates* — so its discontinuities are exactly the polygonal
 * fracture lines the outline is already made of. A crack drawn anywhere else would be a texture
 * of cracks; drawn here, the craze a player watches spread IS the break it is about to get.
 *
 * ⚠️ **IT IS DIEGETIC AND IT IS NOT A METER.** John refused a numeric destruction readout twice
 * and chose *"actively taught"* over *"emergent, no UI"* when asked directly. This is a crack in
 * plaster: no icon, no bar, nothing in screen space, and it is drawn by the wall's own material.
 *
 * ⚠️ **AND IT CANNOT LEAK THE INTERCONNECT.** The A channel is a pure function of 'depth' and
 * never of 'barrier' ('support.js' says why at length), so material in front of the cyan crazes
 * exactly as readily as material in front of the way out.
 */
uniform vec3 uCraze;
...
/**
 * 🧱 **ROUND 12: THE SECTION IS INVISIBLE FROM WHERE THE PLAYER STANDS, AND IT IS THE SAME
 * PROJECTION ARTEFACT ROUND 9 FOUND IN THE LIP — ONE STEP FURTHER IN.**
 *
 * The part-eaten band, i.e. the cut face in plan, is where '|_order - _dmgB| < uCutSoft / 2', so
 * **its width on screen is 'uCutSoft / |grad(_order - _dmgB)|' pixels** — the same quantity
 * 'uRimFloor' exists to floor, applied to the SECTION rather than to the lip. Round 9 fixed the
 * lip and left the section on the raw derivative.
 *
 * 🚨 **AND THE DERIVATIVE IS AT ITS LARGEST AT EXACTLY THE STATE JOHN LOOKS AT.** '_dmgB' is the
 * dug depth mapped through the layer's band; at the END of a dig the crater's interior is
 * saturated at depth 1.0 and the whole falloff from 1.0 to 0 is crammed into the outermost brush
 * radius, so every layer's contour piles onto one cliff. Measured on the shipped frame
 * ('progress/playtest/game.play.st1-dud-cyan-NOW.png', 10x): ornate, then a pale ribbon, then
 * **ONE aliased pixel**, then flat cyan. 'critic-slice-3' read the same frame as *"an ink outline,
 * not a section"* and *"cyan is a flat card with a straight aliased edge"* — those are one defect
 * and this is its arithmetic. Everything round 6 solved (the monotone 0.94 -> 0.45 -> 0.28 -> 0.12
 * ramp), everything round 9 built (the sill/soffit spread) and everything round 10 built (the cyan
 * core in section, 'uCore') is present, correct, and **projected onto one pixel**, which is why
 * ten rounds of section work keep photographing as a contour.
 *
 * 🎯 **THE FLOOR IS IN PIXELS BECAUSE THE DEFECT IS IN PIXELS**, exactly as 'uRimFloor' argues.
 * It raises the EFFECTIVE 'uCutSoft' until the part-eaten band is at least this wide on screen.
 *
 * ⚠️ **IT CANNOT MOVE THE HOLE, AND THAT IS WHY IT IS SAFE.** '_rrrFront''s ramp is CENTRED on
 * '_order == _dmgB' — the file already says it, under uCutSoft: *"the old boundary is the middle
 * of the new ramp, so the hole neither grows nor shrinks."* Widening it symmetrically leaves the
 * 50% contour, i.e. the tear the outline is measured on, exactly where it was. Round 11 had to
 * pay 8 points of cyan fill for its visibility; this pays none.
 *
 * ⚠️ **AND IT CANNOT RESURRECT THE CRATER'S INTERIOR.** '_rrrFront' takes a 'max' with
 * '_rrrSpent', which forces the layer gone above 'uSpent.x' (0.90 of the band) whatever this does
 * — so the widening only ever acts in the ring where the dig is still part-way through the slab,
 * which is the ring that is supposed to BE the section.
 *
 * ⚠️ **IT IS A PAIR, AND THE SECOND HALF IS NOT OPTIONAL — SEE THE CAP'S OWN NOTE BELOW.**
 * '.x' is the width in pixels; '.y' is how far the raise may go, as a multiple of 'uCutSoft'.
 * ⚠️ 0 is the old behaviour to the bit, and it is the default. 'wall.js' asks on band 3 only.
 */
uniform vec2 uSecFloor;      // [minimum section width in px, max raise as a multiple of uCutSoft]
/**
...
float _bgPlate;
// 🧱 see uSecFloor: uCutSoft after the screen-width floor has been applied, held for the whole
// fragment so the march's own _rrrFront calls answer with the same slab the surface does. Set in
// DAMAGE_MAP before anything reads it; identically uCutSoft whenever uSecFloor is 0.
float _bgCutSoft;
...
float _rrrFront(vec2 muv, vec2 duv, float low){
   ...
   */
  // 🧱 '_bgCutSoft', not 'uCutSoft' — see uSecFloor. It is 'uCutSoft' to the bit at the default,
  // and the march has to answer with the same slab the surface does or the cut face it finds
  // would be a different thickness from the one being shaded.
  return max(clamp((b - _rrrOrder(muv, low)) / _bgCutSoft + 0.5, 0.0, 1.0)
           * smoothstep(0.0, _bgCutSoft * 0.8, b),
             _rrrSpent(b));
}
...
const DAMAGE_MAP = /* glsl */ `
        ...
        float _sdG = (uRimFloor > 0.0001 || uSecFloor.x > 0.0001)
          ? length(vec2(dFdx(_sd), dFdy(_sd))) : 0.0;
        /**
         * 🧱 **THE SECTION'S MINIMUM WIDTH ON SCREEN — see uSecFloor.** '_sdG' is how far '_sd'
         * travels per pixel, so 'uSecFloor * _sdG' is the threshold width that buys uSecFloor
         * pixels of part-eaten band. Taken in uniform control flow, above the march: it can only
         * ever WIDEN the ramp, it leaves the ramp's centre — the tear itself — exactly where it
         * was, and it is identically 'uCutSoft' at the default.
         *
         * 🚨 **THE CAP IS NOT TIDINESS AND BOTH OF THIS ROUND'S FIRST TWO BUILDS PROVED IT, IN
         * OPPOSITE DIRECTIONS.** '_order' carries '_bgPlate', a per-cell CONSTANT that STEPS at
         * every plate wall, so '_sdG' there is the whole step in one pixel — it is unbounded, and
         * plate walls run all over the crater's interior, not only round its rim.
         *
         *   · **Uncapped, 7 px:** the ramp widened by more than an order of magnitude at every
         *     plate wall, so the innermost layer stayed part-eaten — i.e. standing, as a
         *     near-black cut face — in big irregular patches ACROSS the crater. Round 4's
         *     islands, in the one colour a crater is not allowed to be.
         *   · **Capped at 3.0x:** dead. Measured 'median 1 -> 1 px, mean 2.97 -> 2.76'. The
         *     collapse this exists to fix happens exactly where '_sdG' is LARGE, and a cap
         *     expressed as a multiple of 'uCutSoft' scales the band by that multiple rather than
         *     setting it — 3x of a third of a pixel is a pixel. **A cap in threshold units
         *     cannot deliver a floor in pixels; that is the whole point of a pixel floor, and
         *     capping it that way silently un-does it.** 'uRimFloor' gets away with 2.4x because
         *     the LIP is already several pixels wide and only pinches at notches.
         *
         * So the cap is real but generous, and it is a UNIFORM so the pair can be swept live in
         * one page on one crater ('harness/scenarios/_th1-section.mjs') instead of one boot per
         * candidate.
         */
        _bgCutSoft = (uSecFloor.x > 0.0001)
          ? clamp(uSecFloor.x * _sdG, uCutSoft, uCutSoft * max(1.0, uSecFloor.y)) : uCutSoft;
        /**
         * 🕸️ **THE STRAIN TELL — see uCraze.** Taken HERE, in uniform control flow, for the same
         * reason '_sdG' is: it needs derivatives, and every fragment reaches this line. Below
         * this point the march discards, and differencing across a discarded neighbour is
         * undefined.
         *
         * A plate wall is a discontinuity, so the lattice steps by a large fraction of its own
         * range across ONE pixel there and by nothing at all inside a plate — which is a
         * one-pixel line at every fracture boundary for one 'fwidth'.
         *
         * ⚠️ **THE UNDER-SAMPLING FADE IS NOT POLISH, IT IS THE DIFFERENCE BETWEEN CRACKS AND A
         * DIRTY WALL.** Once a pixel spans more than about half a plate the step test is true
         * everywhere and the craze becomes a uniform darkening of the whole face — the classic
         * procedural-line failure, and here it would read as "the wall went grey", i.e. a change
         * of material rather than a warning. So it fades out with the sampling rate and the tell
         * is simply absent at a distance where it could not be legible anyway.
         */
        float _craze = 0.0;
        if (uCraze.x > 0.0001 && uPlate > 0.0001) {
          float _pstep = fwidth(_bgPlate) / uPlate;
          float _pp = length(fwidth(vMapUv * uPlateScale));
          _craze = smoothstep(uCraze.y * 0.45, uCraze.y, _pstep)
                 * (1.0 - smoothstep(0.22, 0.55, _pp))
                 * textureLod(tDamage, vDmgUv, 0.0).a * uCraze.x;
        }
        ...
        float _front = max(clamp((_dmgB - _order) / _bgCutSoft + 0.5, 0.0, 1.0)
                     * smoothstep(0.0, _bgCutSoft * 0.8, _dmgB), _rrrSpent(_dmgB));
        /**
        ...
        /**
         * 🕸️ **THE CRAZING GOES ON BEFORE THE LIP AND THE CUT — see uCraze.** It is a property of
         * SURVIVING wall, so it is masked off both of the surfaces that are not that: '_front',
         * the part of this texel already eaten away, and '_cf', the marched cut face, which is
         * fresh break and cannot be under strain because it has already broken.
         *
         * ⚠️ **A MULTIPLY, NOT A MIX TO A CRACK COLOUR.** A crack in plaster is a shadow in a
         * fissure, so it has to scale whatever light the wall has — the same argument the section
         * shading makes two lines below. Mixed to a constant it would glow in an unlit passage
         * and read as an authored decal, which is the "fake tell" wall.js deleted by name.
         */
        if (_craze > 0.0001) diffuseColor.rgb *= 1.0 - _craze * uCraze.z * (1.0 - _front) * (1.0 - _cf);
...
export function applyBreakMask(material, opts = {}) {
  ...
  if (dmg) {
    ...
    /**
     * 🕸️ see uCraze. `[amount, wall threshold, darkness]`, and **x = 0 is inert to the bit** —
     * `wall.js` is the only caller that asks for it, so `chunk-thick.mjs`, `mat-plaster` and
     * every other damage-armed consumer emit the same picture they did before it existed.
     */
    u.uCraze = { value: new THREE.Vector3(...(opts.craze ?? [0, 0.16, 0.55])) };
    ...
    /**
     * 🧱 see uSecFloor. `[px, maxRaise]`. **0 is the pre-round-12 section to the bit**, so bands
     * 0-2, `chunk-thick.mjs` and every other damage-armed caller emit the picture they were
     * scored with. A caller may pass a bare number; the cap then defaults to 6x.
     */
    u.uSecFloor = {
      value: new THREE.Vector2(...(Array.isArray(opts.secFloor)
        ? opts.secFloor : [opts.secFloor ?? 0, 6.0])),
    };
...
${vsrc.slice(vi + 'void main() {'.length)}`;
  ...
  material.customProgramCacheKey = () => (dmg ? 'rrr-break-dmg-v9' : 'rrr-break-v1');
