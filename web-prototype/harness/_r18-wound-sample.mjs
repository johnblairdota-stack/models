/**
 * Critic scratch r18: crop leave wounds and sample cavity meat hex.
 ...
 * Leftover heaps left out of crop. Same boxes as r17 for delta.
 */
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r18-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r18-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r18-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r18-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r18-zoom-fireplace-hole.png');
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r18-wound-fireplace-hole-only.png'), 80, 80, CH, hole);
  const holeImg = { width: 80, height: 80, channels: CH, data: hole };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r18-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
...
{
  const cav = decodePng(readFileSync(path.join(DIR, '_r18-wound-fireplace-cavity.png')));
...
// delta vs archived r17 tight crop
{
  const r17 = decodePng(readFileSync(path.join(DIR, '_r17-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r17 fireplace-cavity-tight ===');
  logStats('r17-tight', sampleBuf(r17.width, r17.height, r17.channels, r17.data));
}

{
  const r17 = decodePng(readFileSync(path.join(DIR, '_r17-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r17 desk-cavity-tight ===');
  logStats('r17-desk-tight', sampleBuf(r17.width, r17.height, r17.channels, r17.data));
}

{
  const bar = decodePng(readFileSync('refs/teardown/yt-house-148s-sledge-hole.png'));
  console.log(`\n=== BAR 148s full ${bar.width}x${bar.height} ===`);
  logStats('148s-full', sampleBuf(bar.width, bar.height, bar.channels, bar.data));
  const tightBar = path.join(DIR, '_r17-bar-148s-hole-tight.png');
  const tb = decodePng(readFileSync(tightBar));
  console.log('\n=== BAR 148s hole-tight (archived crop) ===');
  logStats('148s-hole', sampleBuf(tb.width, tb.height, tb.channels, tb.data));
}
