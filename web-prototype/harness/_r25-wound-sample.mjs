/**
 * Critic scratch r25: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace bite densest meat is the #4c3c2c umber family again (luma ~0.24),
 * not r24's espresso #2f180b luma 0.11 disc. Faces voxel grid, not 113x124 espresso slab.
 * Floor-khaki / robot-white / cyc-grey skipped. Near-black KEPT.
 * Leftover heaps left out of crop. Same boxes as r17–r24 for delta.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r25-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r25-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r25-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r25-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r25-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r25-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r25-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r25-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r25-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r25-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r25-hole-only', holeImg);
}

// archived boxes from r19/r20/r21/r22/r23/r24 on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92], ['r23densest', 78, 62], ['r24densest', 100, 82]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r25-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r25 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r25-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r25-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r25-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r25-tight');
}

{
  const r24 = decodePng(readFileSync(path.join(DIR, '_r24-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r24 fireplace-cavity-tight ===');
  logStats('r24-tight', sampleBuf(r24.width, r24.height, r24.channels, r24.data));
  countGrain(r24.width, r24.height, r24.channels, r24.data, 'r24-tight');
  meanRun(r24.width, r24.height, r24.channels, r24.data, 'r24-tight');
}

{
  const r24 = decodePng(readFileSync(path.join(DIR, '_r24-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r24 fireplace-meat-window ===');
  logStats('r24-meat-window', sampleBuf(r24.width, r24.height, r24.channels, r24.data));
  countGrain(r24.width, r24.height, r24.channels, r24.data, 'r24-meat-window');
  meanRun(r24.width, r24.height, r24.channels, r24.data, 'r24-meat-window');
}

{
  const r24h = decodePng(readFileSync(path.join(DIR, '_r24-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r24 fireplace-hole-only ===');
  logStats('r24-hole-only', sampleBuf(r24h.width, r24h.height, r24h.channels, r24h.data));
  countGrain(r24h.width, r24h.height, r24h.channels, r24h.data, 'r24-hole-only');
  meanRun(r24h.width, r24h.height, r24h.channels, r24h.data, 'r24-hole-only');
}

{
  const r24d = decodePng(readFileSync(path.join(DIR, '_r24-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r24 desk-cavity-tight ===');
  logStats('r24-desk-tight', sampleBuf(r24d.width, r24d.height, r24d.channels, r24d.data));
}

hueSplit('r25-tight', tight);
hueSplit('r25-meat-window', decodePng(readFileSync(path.join(DIR, '_r25-wound-fireplace-meat-window.png'))));
hueSplit('r24-tight', decodePng(readFileSync(path.join(DIR, '_r24-wound-fireplace-cavity-tight.png'))));
hueSplit('r24-meat-window', decodePng(readFileSync(path.join(DIR, '_r24-wound-fireplace-meat-window.png'))));
hueSplit('r24-hole-only', decodePng(readFileSync(path.join(DIR, '_r24-wound-fireplace-hole-only.png'))));
hueSplit('r25-densest-crop', decodePng(readFileSync(path.join(DIR, '_r25-wound-fireplace-hole-only.png'))));
