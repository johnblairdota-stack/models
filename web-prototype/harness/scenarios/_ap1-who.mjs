/**
 * 🕳️ **WHO OWNS THE PIXELS THAT ARE STILL BLACK?** — every hit along the ray, in depth order.
 *
 *   node harness/playtest.mjs --view game.play --script harness/scenarios/_ap1-who.mjs \
 *        --port 5455 --q "seed=s4" --shots
 *
 * Two differences from `harness/_tmp_geoprobe.mjs --pick`, and both were load-bearing here:
 *   1. **Instances are expanded.** An `InstancedMesh`'s `matrixWorld` is its parent's, so every
 *      copy it draws is invisible to a probe that only reads that — and `wallinstances.js`
 *      draws every pristine wall face in the house out of four of them.
 *   2. **Facing is respected.** A raycast does not cull, so it happily reports a mesh the GPU
 *      threw away — which is how a one-sided panel reads as "present" to a probe and as a black
 *      hole on screen. Each hit is tagged FRONT/BACK and marked DROPPED when the material's own
 *      `side` would have discarded it.
 * A single nearest-hit answer cannot distinguish "nothing draws here" from "something black
 * draws here", so this prints the whole list.
 */
export default async function ap1who({ page, note, skip, shot }) {
  ...
  for (const id of ['x.ballroom.orangery', 'x.gallery.east', 'x.ballroom.terrace_e']) {
    const s = await park(id, 0.6);
    if (s?.space !== s?.home) { note(`${id}: parked in ${s?.space}, wanted ${s?.home} — skipped`); continue; }
    await settle(25);
    await shot(`ap1-who-${id.replace(/\./g, '_')}`);
    for (const [u, v] of [[0.5, 0.5], [0.35, 0.7]]) {
      const r = await rayAt(id, u, v);
      note(`${id} uv ${u},${v} px ${r.px.join(',')} — ${r.total} intersections`);
      for (const h of r.list) {
        note(`    ${h.drawn ? 'DRAWN  ' : 'dropped'} d=${String(h.dist).padStart(7)} ${h.front ? 'front' : 'back '} side=${h.side} ${h.name} [${h.mat.slice(0, 46)}]`);
      }
    }
  }
