...
async function runCombo(id, buildBallots, opts = {}) {
  const CODE = 'n6' + Math.random().toString(36).slice(2, 5);
  const { tv, phones, ids } = await seatN(CODE, 6);
  ...
  const row = { id, CODE, N: 6, ids, ballots, serverVotes: lastBallots?.votes, pair, castEv: castEv?.data, expected, pass, note: opts.note || '' };
  results.push(row);
...
await runCombo('N6-unanimous', (ids) => ids.map(() => ({ runner: ids[0], guide: ids[1] })));
await runCombo('N6-near-tie-3-3', (ids) => [
  { runner: ids[0], guide: ids[5] },
  { runner: ids[0], guide: ids[5] },
  { runner: ids[0], guide: ids[5] },
  { runner: ids[1], guide: ids[5] },
  { runner: ids[1], guide: ids[5] },
  { runner: ids[1], guide: ids[5] },
], { note: 'runner 3-3 → seeded' });
await runCombo('N6-plurality', (ids) => [
...
await runCombo('N6-multiway-split', (ids) => ids.map((_, i) => ({
  runner: ids[i], guide: ids[(i + 1) % 6],
})), { note: '6-way runner split → seeded' });
await runCombo('N6-self-pick-void', (ids) => [
...
await runCombo('N6-partial', (ids) => [
  ...
  { runner: ids[0], guide: ids[1] },
], { note: 'partials dropped' });
await runCombo('N6-role-conflict', (ids) => [
  { runner: ids[0], guide: ids[1] },
  { runner: ids[0], guide: ids[2] },
...
], { note: 'Ada runner; guide Ada excluded → next scores' });

{
  const CODE = 'n6e' + Math.random().toString(36).slice(2, 4);
  const { tv, phones } = await seatN(CODE, 6);
  send(tv, { t: 'episode', opts: {} });
  await sleep(350);
  const last = tv.msgs.filter((m) => m.t === 'state').at(-1);
  const row = { id: 'N6-empty-noop', N: 6, pair: last?.frame?.pair || null, phase: last?.frame?.phase, pass: !last?.frame?.pair?.runner, note: 'empty+unused must not invent' };
...
await writeFile('progress/overnight-vote/loop2-n6-wire.json', JSON.stringify({ at: new Date().toISOString(), head, N: 6, results }, null, 2));
const fails = results.filter((r) => !r.pass).length;
console.log('WIRE_SUMMARY_N6', { pass: results.length - fails, fail: fails, total: results.length, head });
