/**
 * Critic scratch r25 pass 2: 148s densest 80x80 vs r25 hole.
...
for (const b of boxes) {
  ...
  writePng(path.join(DIR, `_r25-bar-${b.tag}.png`), b.w, b.h, crop.ch, crop.buf);
  console.log(`\n=== BAR ${b.tag} (no skip) ===`);
...
{
  const tightBar = decodePng(readFileSync(path.join(DIR, '_r25-bar-148s-hole-tight.png')));
  const z = nearestZoom(tightBar, 4);
  writePng(path.join(DIR, '_r25-zoom-148s-hole-tight.png'), z.w, z.h, z.ch, z.data);
  const best = densest(tightBar, '148s-tight');
  const hole = cropXY(tightBar, best.x, best.y, 80, 80);
  writePng(path.join(DIR, '_r25-bar-148s-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const hz = nearestZoom({ width: 80, height: 80, channels: hole.ch, data: hole.buf }, 4);
  writePng(path.join(DIR, '_r25-zoom-148s-hole-only.png'), hz.w, hz.h, hz.ch, hz.data);
...

{
  ...
  console.log('\n=== r25 densest 80x80 re-sample ===');
  logStats('r25-hole', sampleBuf(r25h.width, r25h.height, r25h.channels, r25h.data));
  hueSplit('r25-hole-only', r25h);
}

{
  const r24h = decodePng(readFileSync(path.join(DIR, '_r24-wound-fireplace-hole-only.png')));
  console.log('\n=== archived r24 densest 80x80 ===');
  logStats('r24-hole', sampleBuf(r24h.width, r24h.height, r24h.channels, r24h.data));
  hueSplit('r24-hole-only', r24h);
}

{
  const r23h = decodePng(readFileSync(path.join(DIR, '_r23-wound-fireplace-hole-only.png')));
  console.log('\n=== archived r23 densest 80x80 ===');
  logStats('r23-hole', sampleBuf(r23h.width, r23h.height, r23h.channels, r23h.data));
  hueSplit('r23-hole-only', r23h);
}
