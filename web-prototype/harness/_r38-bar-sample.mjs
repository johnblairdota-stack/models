/**
 * Critic scratch r38 pass 2: 148s densest 80x80 vs r38 hole.
...
for (const b of boxes) {
  ...
  writePng(path.join(DIR, `_r38-bar-${b.tag}.png`), b.w, b.h, crop.ch, crop.buf);
  console.log(`\n=== BAR ${b.tag} (no skip) ===`);
...
{
  const tightBar = decodePng(readFileSync(path.join(DIR, '_r38-bar-148s-hole-tight.png')));
  const z = nearestZoom(tightBar, 4);
  writePng(path.join(DIR, '_r38-zoom-148s-hole-tight.png'), z.w, z.h, z.ch, z.data);
  const best = densest(tightBar, '148s-tight');
  const hole = cropXY(tightBar, best.x, best.y, 80, 80);
  writePng(path.join(DIR, '_r38-bar-148s-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const hz = nearestZoom({ width: 80, height: 80, channels: hole.ch, data: hole.buf }, 4);
  writePng(path.join(DIR, '_r38-zoom-148s-hole-only.png'), hz.w, hz.h, hz.ch, hz.data);
...
{
  const r38h = decodePng(readFileSync(path.join(DIR, '_r38-wound-fireplace-hole-only.png')));
  console.log('\n=== r38 densest 80x80 re-sample ===');
  logStats('r38-hole', sampleBuf(r38h.width, r38h.height, r38h.channels, r38h.data));
  hueSplit('r38-hole-only', r38h);
}

{
  const r37h = decodePng(readFileSync(path.join(DIR, '_r37-wound-fireplace-hole-only.png')));
  console.log('\n=== archived r37 densest 80x80 ===');
  logStats('r37-hole', sampleBuf(r37h.width, r37h.height, r37h.channels, r37h.data));
  hueSplit('r37-hole-only', r37h);
}

{
  const r36h = decodePng(readFileSync(path.join(DIR, '_r36-wound-fireplace-hole-only.png')));
  console.log('\n=== archived r36 densest 80x80 ===');
  logStats('r36-hole', sampleBuf(r36h.width, r36h.height, r36h.channels, r36h.data));
  hueSplit('r36-hole-only', r36h);
}

{
  const r35h = decodePng(readFileSync(path.join(DIR, '_r35-wound-fireplace-hole-only.png')));
  console.log('\n=== archived r35 densest 80x80 ===');
  logStats('r35-hole', sampleBuf(r35h.width, r35h.height, r35h.channels, r35h.data));
  hueSplit('r35-hole-only', r35h);
}

{
  const r34h = decodePng(readFileSync(path.join(DIR, '_r34-wound-fireplace-hole-only.png')));
  console.log('\n=== archived r34 densest 80x80 ===');
  logStats('r34-hole', sampleBuf(r34h.width, r34h.height, r34h.channels, r34h.data));
  hueSplit('r34-hole-only', r34h);
}

{
  const r33h = decodePng(readFileSync(path.join(DIR, '_r33-wound-fireplace-hole-only.png')));
  console.log('\n=== archived r33 densest 80x80 ===');
  logStats('r33-hole', sampleBuf(r33h.width, r33h.height, r33h.channels, r33h.data));
  hueSplit('r33-hole-only', r33h);
}

{
  const r32h = decodePng(readFileSync(path.join(DIR, '_r32-wound-fireplace-hole-only.png')));
  console.log('\n=== archived r32 densest 80x80 ===');
