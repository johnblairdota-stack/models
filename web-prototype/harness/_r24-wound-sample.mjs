/**
 * Critic scratch r24: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace bite densest meat is one packed umber island through the volume
 * (148s #4c3c2c family, lip-band faces), not an umber/dust/espresso split.
 * Espresso gaps must not eat the lip. Faces still voxels, not 79x70 slabs.
 * Floor-khaki / robot-white / cyc-grey skipped. Near-black KEPT.
 * Leftover heaps left out of crop. Same boxes as r17–r23 for delta.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r24-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r24-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r24-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r24-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r24-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r24-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r24-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r24-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r24-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r24-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r24-hole-only', holeImg);
}

// archived boxes from r19/r20/r21/r22/r23 on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92], ['r23densest', 78, 62]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r24-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r24 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r24-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r24-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r24-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r24-tight');
}

{
  const r23 = decodePng(readFileSync(path.join(DIR, '_r23-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r23 fireplace-cavity-tight ===');
  logStats('r23-tight', sampleBuf(r23.width, r23.height, r23.channels, r23.data));
  countGrain(r23.width, r23.height, r23.channels, r23.data, 'r23-tight');
  meanRun(r23.width, r23.height, r23.channels, r23.data, 'r23-tight');
}

{
  const r23 = decodePng(readFileSync(path.join(DIR, '_r23-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r23 fireplace-meat-window ===');
  logStats('r23-meat-window', sampleBuf(r23.width, r23.height, r23.channels, r23.data));
  countGrain(r23.width, r23.height, r23.channels, r23.data, 'r23-meat-window');
  meanRun(r23.width, r23.height, r23.channels, r23.data, 'r23-meat-window');
}

{
  const r23h = decodePng(readFileSync(path.join(DIR, '_r23-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r23 fireplace-hole-only ===');
  logStats('r23-hole-only', sampleBuf(r23h.width, r23h.height, r23h.channels, r23h.data));
  countGrain(r23h.width, r23h.height, r23h.channels, r23h.data, 'r23-hole-only');
  meanRun(r23h.width, r23h.height, r23h.channels, r23h.data, 'r23-hole-only');
}

{
  const r23d = decodePng(readFileSync(path.join(DIR, '_r23-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r23 desk-cavity-tight ===');
  logStats('r23-desk-tight', sampleBuf(r23d.width, r23d.height, r23d.channels, r23d.data));
}

hueSplit('r24-tight', tight);
hueSplit('r24-meat-window', decodePng(readFileSync(path.join(DIR, '_r24-wound-fireplace-meat-window.png'))));
hueSplit('r23-tight', decodePng(readFileSync(path.join(DIR, '_r23-wound-fireplace-cavity-tight.png'))));
hueSplit('r23-meat-window', decodePng(readFileSync(path.join(DIR, '_r23-wound-fireplace-meat-window.png'))));
hueSplit('r23-hole-only', decodePng(readFileSync(path.join(DIR, '_r23-wound-fireplace-hole-only.png'))));
