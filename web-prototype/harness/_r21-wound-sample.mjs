/**
 * Critic scratch r21: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace bite interior is a packed same-hue voxel island you can read into,
 * not a rim then a few thick vertical slabs.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r21-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r21-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r21-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r21-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r21-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r21-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r21-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r21-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r21-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r21-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
...
// archived boxes from r19/r20 on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88]]) {
    ...
    writePng(path.join(DIR, `_r21-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r21 tight ===`);
...
{
  const cav = decodePng(readFileSync(path.join(DIR, '_r21-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r21-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r21-tight');
}

{
  const r20 = decodePng(readFileSync(path.join(DIR, '_r20-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r20 fireplace-cavity-tight ===');
  logStats('r20-tight', sampleBuf(r20.width, r20.height, r20.channels, r20.data));
  countGrain(r20.width, r20.height, r20.channels, r20.data, 'r20-tight');
  meanRun(r20.width, r20.height, r20.channels, r20.data, 'r20-tight');
}

{
  const r20 = decodePng(readFileSync(path.join(DIR, '_r20-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r20 fireplace-meat-window ===');
  logStats('r20-meat-window', sampleBuf(r20.width, r20.height, r20.channels, r20.data));
  countGrain(r20.width, r20.height, r20.channels, r20.data, 'r20-meat-window');
  meanRun(r20.width, r20.height, r20.channels, r20.data, 'r20-meat-window');
}

{
  const r20h = decodePng(readFileSync(path.join(DIR, '_r20-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r20 fireplace-hole-only ===');
  logStats('r20-hole-only', sampleBuf(r20h.width, r20h.height, r20h.channels, r20h.data));
  countGrain(r20h.width, r20h.height, r20h.channels, r20h.data, 'r20-hole-only');
  meanRun(r20h.width, r20h.height, r20h.channels, r20h.data, 'r20-hole-only');
}

{
  const r20d = decodePng(readFileSync(path.join(DIR, '_r20-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r20 desk-cavity-tight ===');
  logStats('r20-desk-tight', sampleBuf(r20d.width, r20d.height, r20d.channels, r20d.data));
}
