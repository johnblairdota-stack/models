/**
 * Critic scratch r20: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace bite chip grain is a dense house-voxel island, not a handful of large cubes.
 * Floor-khaki / robot-white / cyc-grey skipped. Near-black KEPT.
 * Leftover heaps left out of crop. Same boxes as r17/r18/r19 for delta.
 */
...
/** Count distinct cube-face blobs via 4-connected quantized-luma components of chromatic meat. */
function countGrain(W, H, CH, px, tag) {
  ...
  const C = (xx, yy) => {
    const i = (yy * W + xx) * CH;
    return chroma([px[i], px[i + 1], px[i + 2]]);
  };
  const meat = (xx, yy) => {
    const l = L(xx, yy), c = C(xx, yy);
    if (l > 0.40 && c < 0.14) return false;
    if (l > 0.72) return false;
    return c >= 0.08 && l >= 0.10 && l < 0.45;
  };
    ...
    for (let x = 0; x < W; x++) {
      ...
      while (stack.length) {
        ...
        for (const [nx, ny] of nbs) {
          ...
          const ni = ny * W + nx;
          if (seen[ni] || !meat(nx, ny)) continue;
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r20-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r20-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r20-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r20-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r20-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r20-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r20-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r20-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r20-wound-fireplace-hole-only.png'), 80, 80, CH, hole);
  const holeImg = { width: 80, height: 80, channels: CH, data: hole };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r20-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  console.log('wrote hole-only + 4x');
  console.log('\n=== fireplace-hole-only ===');
  logStats('fireplace-hole-only', sampleBuf(80, 80, CH, hole));
  countGrain(80, 80, CH, hole, 'fireplace-hole-only');
}

// row scan of chromatic lip in cavity
{
  const cav = decodePng(readFileSync(path.join(DIR, '_r20-wound-fireplace-cavity.png')));
...
{
  const r19 = decodePng(readFileSync(path.join(DIR, '_r19-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r19 fireplace-cavity-tight ===');
  logStats('r19-tight', sampleBuf(r19.width, r19.height, r19.channels, r19.data));
  countGrain(r19.width, r19.height, r19.channels, r19.data, 'r19-tight');
}

{
  const r19 = decodePng(readFileSync(path.join(DIR, '_r19-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r19 desk-cavity-tight ===');
  logStats('r19-desk-tight', sampleBuf(r19.width, r19.height, r19.channels, r19.data));
}

{
  const r19 = decodePng(readFileSync(path.join(DIR, '_r19-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r19 fireplace-meat-window ===');
  logStats('r19-meat-window', sampleBuf(r19.width, r19.height, r19.channels, r19.data));
  countGrain(r19.width, r19.height, r19.channels, r19.data, 'r19-meat-window');
}

{
  const r19h = decodePng(readFileSync(path.join(DIR, '_r19-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r19 fireplace-hole-only ===');
  logStats('r19-hole-only', sampleBuf(r19h.width, r19h.height, r19h.channels, r19h.data));
  countGrain(r19h.width, r19h.height, r19h.channels, r19h.data, 'r19-hole-only');
}

{
  const tightNow = decodePng(readFileSync(tightPath));
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r20-tight');
}
