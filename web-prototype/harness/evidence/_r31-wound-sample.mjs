/**
 * Critic scratch r31: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace densest 80x80 is a packed umber island again (lip back
 * toward r28's 73.9%, #4c3c2c family, not khaki #5c554a through a sparse
 * grid). Voxel grid not 104x113. Chaise stays a seat-shaped bite, not a brown
 * air sphere. You can still read meat through the fireplace volume (not a
 * black well). Meat must not have gone cream. Chaise / settee / table-round /
 * crate plates must not have gone light.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r31-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r31-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r31-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r31-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r31-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r31-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r31-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r31-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r31-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r31-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r31-hole-only', holeImg);
}

// archived boxes from r19–r30 densest coords on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92], ['r23densest', 78, 62], ['r24densest', 100, 82], ['r25densest', 100, 82], ['r26densest', 100, 82], ['r27densest', 79, 62], ['r28densest', 81, 61], ['r29densest', 81, 59], ['r30densest', 100, 100]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r31-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r31 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r31-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r31-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r31-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r31-tight');
}

{
  const r30 = decodePng(readFileSync(path.join(DIR, '_r30-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r30 fireplace-cavity-tight ===');
  logStats('r30-tight', sampleBuf(r30.width, r30.height, r30.channels, r30.data));
  countGrain(r30.width, r30.height, r30.channels, r30.data, 'r30-tight');
  meanRun(r30.width, r30.height, r30.channels, r30.data, 'r30-tight');
}

{
  const r30 = decodePng(readFileSync(path.join(DIR, '_r30-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r30 fireplace-meat-window ===');
  logStats('r30-meat-window', sampleBuf(r30.width, r30.height, r30.channels, r30.data));
  countGrain(r30.width, r30.height, r30.channels, r30.data, 'r30-meat-window');
  meanRun(r30.width, r30.height, r30.channels, r30.data, 'r30-meat-window');
}

{
  const r30h = decodePng(readFileSync(path.join(DIR, '_r30-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r30 fireplace-hole-only ===');
  logStats('r30-hole-only', sampleBuf(r30h.width, r30h.height, r30h.channels, r30h.data));
  countGrain(r30h.width, r30h.height, r30h.channels, r30h.data, 'r30-hole-only');
  meanRun(r30h.width, r30h.height, r30h.channels, r30h.data, 'r30-hole-only');
}

{
  const r30d = decodePng(readFileSync(path.join(DIR, '_r30-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r30 desk-cavity-tight ===');
  logStats('r30-desk-tight', sampleBuf(r30d.width, r30d.height, r30d.channels, r30d.data));
}

hueSplit('r31-tight', tight);
hueSplit('r31-meat-window', decodePng(readFileSync(path.join(DIR, '_r31-wound-fireplace-meat-window.png'))));
hueSplit('r30-tight', decodePng(readFileSync(path.join(DIR, '_r30-wound-fireplace-cavity-tight.png'))));
hueSplit('r30-meat-window', decodePng(readFileSync(path.join(DIR, '_r30-wound-fireplace-meat-window.png'))));
hueSplit('r30-hole-only', decodePng(readFileSync(path.join(DIR, '_r30-wound-fireplace-hole-only.png'))));
hueSplit('r31-densest-crop', decodePng(readFileSync(path.join(DIR, '_r31-wound-fireplace-hole-only.png'))));
