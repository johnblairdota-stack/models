/**
 * Critic scratch r23: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace bite densest meat is one crumbled umber island through the volume
 * (same hue as Teardown 148s #4c3c2c family, lip-band faces), not a terracotta/dust split.
 * Floor-khaki / robot-white / cyc-grey skipped. Near-black KEPT.
 * Leftover heaps left out of crop. Same boxes as r17–r22 for delta.
...
function hueSplit(tag, img) {
  const { width: W, height: H, channels: CH, data: px } = img;
  let nKeep = 0, terra = 0, umber = 0, dust = 0;
  for (let i = 0; i < W * H; i++) {
    ...
    nKeep++;
    if (C >= 0.08 && L >= 0.13 && L < 0.18) terra++;
    else if (C >= 0.08 && L >= 0.20 && L < 0.28) umber++;
    else if (C >= 0.08 && L >= 0.28 && L < 0.36) dust++;
  }
  const pct = (n) => (nKeep ? (100 * n / nKeep).toFixed(1) : 'na');
  console.log(`HUE-SPLIT ${tag} keep=${nKeep} terra0.13-0.18=${pct(terra)}% umber0.20-0.28=${pct(umber)}% dust0.28-0.36=${pct(dust)}%`);
}
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r23-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r23-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r23-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r23-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r23-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r23-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r23-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r23-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r23-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r23-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r23-hole-only', holeImg);
}

// archived boxes from r19/r20/r21/r22 on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r23-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r23 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r23-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r23-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r23-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r23-tight');
}

{
  const r22 = decodePng(readFileSync(path.join(DIR, '_r22-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r22 fireplace-cavity-tight ===');
  logStats('r22-tight', sampleBuf(r22.width, r22.height, r22.channels, r22.data));
  countGrain(r22.width, r22.height, r22.channels, r22.data, 'r22-tight');
  meanRun(r22.width, r22.height, r22.channels, r22.data, 'r22-tight');
}

{
  const r22 = decodePng(readFileSync(path.join(DIR, '_r22-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r22 fireplace-meat-window ===');
  logStats('r22-meat-window', sampleBuf(r22.width, r22.height, r22.channels, r22.data));
  countGrain(r22.width, r22.height, r22.channels, r22.data, 'r22-meat-window');
  meanRun(r22.width, r22.height, r22.channels, r22.data, 'r22-meat-window');
}

{
  const r22h = decodePng(readFileSync(path.join(DIR, '_r22-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r22 fireplace-hole-only ===');
  logStats('r22-hole-only', sampleBuf(r22h.width, r22h.height, r22h.channels, r22h.data));
  countGrain(r22h.width, r22h.height, r22h.channels, r22h.data, 'r22-hole-only');
  meanRun(r22h.width, r22h.height, r22h.channels, r22h.data, 'r22-hole-only');
}

{
  const r22d = decodePng(readFileSync(path.join(DIR, '_r22-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r22 desk-cavity-tight ===');
  logStats('r22-desk-tight', sampleBuf(r22d.width, r22d.height, r22d.channels, r22d.data));
}

hueSplit('r23-tight', tight);
hueSplit('r23-meat-window', decodePng(readFileSync(path.join(DIR, '_r23-wound-fireplace-meat-window.png'))));
hueSplit('r22-tight', decodePng(readFileSync(path.join(DIR, '_r22-wound-fireplace-cavity-tight.png'))));
hueSplit('r22-meat-window', decodePng(readFileSync(path.join(DIR, '_r22-wound-fireplace-meat-window.png'))));
hueSplit('r22-hole-only', decodePng(readFileSync(path.join(DIR, '_r22-wound-fireplace-hole-only.png'))));
