/**
 * Critic scratch r32: crop leave wounds, sample cavity meat hex, count chip grain.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r32-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r32-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r32-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r32-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r32-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r32-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r32-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r32-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r32-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r32-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r32-hole-only', holeImg);
}

// archived boxes from r19–r30 densest coords on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92], ['r23densest', 78, 62], ['r24densest', 100, 82], ['r25densest', 100, 82], ['r26densest', 100, 82], ['r27densest', 79, 62], ['r28densest', 81, 61], ['r29densest', 81, 59], ['r30densest', 100, 100], ['r31densest', 91, 73]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r32-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r32 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r32-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r32-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r32-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r32-tight');
}

{
  const r31 = decodePng(readFileSync(path.join(DIR, '_r31-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r31 fireplace-cavity-tight ===');
  logStats('r31-tight', sampleBuf(r31.width, r31.height, r31.channels, r31.data));
  countGrain(r31.width, r31.height, r31.channels, r31.data, 'r31-tight');
  meanRun(r31.width, r31.height, r31.channels, r31.data, 'r31-tight');
}

{
  const r31 = decodePng(readFileSync(path.join(DIR, '_r31-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r31 fireplace-meat-window ===');
  logStats('r31-meat-window', sampleBuf(r31.width, r31.height, r31.channels, r31.data));
  countGrain(r31.width, r31.height, r31.channels, r31.data, 'r31-meat-window');
  meanRun(r31.width, r31.height, r31.channels, r31.data, 'r31-meat-window');
}

{
  const r31h = decodePng(readFileSync(path.join(DIR, '_r31-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r31 fireplace-hole-only ===');
  logStats('r31-hole-only', sampleBuf(r31h.width, r31h.height, r31h.channels, r31h.data));
  countGrain(r31h.width, r31h.height, r31h.channels, r31h.data, 'r31-hole-only');
  meanRun(r31h.width, r31h.height, r31h.channels, r31h.data, 'r31-hole-only');
}

{
  const r31d = decodePng(readFileSync(path.join(DIR, '_r31-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r31 desk-cavity-tight ===');
  logStats('r31-desk-tight', sampleBuf(r31d.width, r31d.height, r31d.channels, r31d.data));
}

hueSplit('r32-tight', tight);
hueSplit('r32-meat-window', decodePng(readFileSync(path.join(DIR, '_r32-wound-fireplace-meat-window.png'))));
hueSplit('r31-tight', decodePng(readFileSync(path.join(DIR, '_r31-wound-fireplace-cavity-tight.png'))));
hueSplit('r31-meat-window', decodePng(readFileSync(path.join(DIR, '_r31-wound-fireplace-meat-window.png'))));
hueSplit('r31-hole-only', decodePng(readFileSync(path.join(DIR, '_r31-wound-fireplace-hole-only.png'))));
hueSplit('r32-densest-crop', decodePng(readFileSync(path.join(DIR, '_r32-wound-fireplace-hole-only.png'))));
