/**
 * Critic scratch r17: crop leave wounds and sample cavity meat hex.
 * Claim: stepped same-hue lip at luma ~0.18, not dead-black punch, not cream.
 * Floor-khaki / robot-white skipped. Near-black meat is KEPT.
...
function sample(img, x, y, w, h) {
  ...
  for (let yy = 0; yy < h; yy++) {
    for (let xx = 0; xx < w; xx++) {
      ...
      if (floorKhaki || robotWhite || cycGrey) {
        nSkip++;
        hot[ho] = 0; hot[ho + 1] = 0; hot[ho + 2] = 0;
        continue;
      }
      nKeep++;
      ...
      buckets.set(k, rec);
      // magenta = cream, cyan = readable lip band, dim red = punch-black, grey = other keep
  ...
  const ranked = [...buckets.entries()]
    ...
    .sort((a, b) => b.n - a.n)
    .slice(0, 12);
  return { nKeep, nSkip, nCream, nDark, nMid, nLip, nPunch, ranked, crop, hot, ch: CH };
}
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r17-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r17-hot-${tag}.png`);
