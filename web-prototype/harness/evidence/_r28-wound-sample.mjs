/**
 * Critic scratch r28: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace bite densest 80x80 is one umber island through the volume
 * (148s #4c3c2c family, lip-band faces), not an umber/dust split. Faces still
 * a voxel grid, not a 104x113 espresso slab. You can still read meat through
 * the volume (not a black well). Meat must not have gone cream. Flying plates
 * / bodyTint pick are not in the claim. Chaise / settee / table-round / crate
 * plates must not have gone light.
 * Floor-khaki / robot-white / cyc-grey skipped. Near-black KEPT.
 * Leftover heaps left out of crop. Same boxes as r17–r27 for delta.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r28-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r28-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r28-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r28-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r28-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r28-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r28-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r28-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r28-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r28-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r28-hole-only', holeImg);
}

// archived boxes from r19/r20/r21/r22/r23/r24/r25/r26/r27 on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92], ['r23densest', 78, 62], ['r24densest', 100, 82], ['r25densest', 100, 82], ['r26densest', 100, 82], ['r27densest', 79, 62]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r28-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r28 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r28-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r28-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r28-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r28-tight');
}

{
  const r27 = decodePng(readFileSync(path.join(DIR, '_r27-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r27 fireplace-cavity-tight ===');
  logStats('r27-tight', sampleBuf(r27.width, r27.height, r27.channels, r27.data));
  countGrain(r27.width, r27.height, r27.channels, r27.data, 'r27-tight');
  meanRun(r27.width, r27.height, r27.channels, r27.data, 'r27-tight');
}

{
  const r27 = decodePng(readFileSync(path.join(DIR, '_r27-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r27 fireplace-meat-window ===');
  logStats('r27-meat-window', sampleBuf(r27.width, r27.height, r27.channels, r27.data));
  countGrain(r27.width, r27.height, r27.channels, r27.data, 'r27-meat-window');
  meanRun(r27.width, r27.height, r27.channels, r27.data, 'r27-meat-window');
}

{
  const r27h = decodePng(readFileSync(path.join(DIR, '_r27-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r27 fireplace-hole-only ===');
  logStats('r27-hole-only', sampleBuf(r27h.width, r27h.height, r27h.channels, r27h.data));
  countGrain(r27h.width, r27h.height, r27h.channels, r27h.data, 'r27-hole-only');
  meanRun(r27h.width, r27h.height, r27h.channels, r27h.data, 'r27-hole-only');
}

{
  const r27d = decodePng(readFileSync(path.join(DIR, '_r27-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r27 desk-cavity-tight ===');
  logStats('r27-desk-tight', sampleBuf(r27d.width, r27d.height, r27d.channels, r27d.data));
}

hueSplit('r28-tight', tight);
hueSplit('r28-meat-window', decodePng(readFileSync(path.join(DIR, '_r28-wound-fireplace-meat-window.png'))));
hueSplit('r27-tight', decodePng(readFileSync(path.join(DIR, '_r27-wound-fireplace-cavity-tight.png'))));
hueSplit('r27-meat-window', decodePng(readFileSync(path.join(DIR, '_r27-wound-fireplace-meat-window.png'))));
hueSplit('r27-hole-only', decodePng(readFileSync(path.join(DIR, '_r27-wound-fireplace-hole-only.png'))));
hueSplit('r28-densest-crop', decodePng(readFileSync(path.join(DIR, '_r28-wound-fireplace-hole-only.png'))));
