/**
 * Critic scratch r26: crop leave wounds, sample cavity meat hex, count chip grain.
 ...
 * Leftover heaps left out of crop. Same boxes as r17–r25 for delta.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r26-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r26-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r26-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r26-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r26-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r26-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r26-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r26-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r26-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r26-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r26-hole-only', holeImg);
}

// archived boxes from r19/r20/r21/r22/r23/r24/r25 on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92], ['r23densest', 78, 62], ['r24densest', 100, 82], ['r25densest', 100, 82]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r26-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r26 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r26-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r26-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r26-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r26-tight');
}

{
  const r25 = decodePng(readFileSync(path.join(DIR, '_r25-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r25 fireplace-cavity-tight ===');
  logStats('r25-tight', sampleBuf(r25.width, r25.height, r25.channels, r25.data));
  countGrain(r25.width, r25.height, r25.channels, r25.data, 'r25-tight');
  meanRun(r25.width, r25.height, r25.channels, r25.data, 'r25-tight');
}

{
  const r25 = decodePng(readFileSync(path.join(DIR, '_r25-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r25 fireplace-meat-window ===');
  logStats('r25-meat-window', sampleBuf(r25.width, r25.height, r25.channels, r25.data));
  countGrain(r25.width, r25.height, r25.channels, r25.data, 'r25-meat-window');
  meanRun(r25.width, r25.height, r25.channels, r25.data, 'r25-meat-window');
}

{
  const r25h = decodePng(readFileSync(path.join(DIR, '_r25-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r25 fireplace-hole-only ===');
  logStats('r25-hole-only', sampleBuf(r25h.width, r25h.height, r25h.channels, r25h.data));
  countGrain(r25h.width, r25h.height, r25h.channels, r25h.data, 'r25-hole-only');
  meanRun(r25h.width, r25h.height, r25h.channels, r25h.data, 'r25-hole-only');
}

{
  const r25d = decodePng(readFileSync(path.join(DIR, '_r25-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r25 desk-cavity-tight ===');
  logStats('r25-desk-tight', sampleBuf(r25d.width, r25d.height, r25d.channels, r25d.data));
}

hueSplit('r26-tight', tight);
hueSplit('r26-meat-window', decodePng(readFileSync(path.join(DIR, '_r26-wound-fireplace-meat-window.png'))));
hueSplit('r25-tight', decodePng(readFileSync(path.join(DIR, '_r25-wound-fireplace-cavity-tight.png'))));
hueSplit('r25-meat-window', decodePng(readFileSync(path.join(DIR, '_r25-wound-fireplace-meat-window.png'))));
hueSplit('r25-hole-only', decodePng(readFileSync(path.join(DIR, '_r25-wound-fireplace-hole-only.png'))));
hueSplit('r26-densest-crop', decodePng(readFileSync(path.join(DIR, '_r26-wound-fireplace-hole-only.png'))));
