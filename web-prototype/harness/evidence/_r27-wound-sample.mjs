/**
 * Critic scratch r27: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace bite densest meat is the #4c3c2c umber family (luma ~0.24),
 * not espresso #2e170a / #30190b luma ~0.11 disc. Faces read as a voxel grid,
 * not a 104x113 espresso slab. You can still read meat through the volume
 * (not a black well). Meat must not have gone cream. Flying plates / bodyTint
 * pick are not in the claim. Chaise / settee / table-round / crate plates must
 * not have gone light.
 * Floor-khaki / robot-white / cyc-grey skipped. Near-black KEPT.
 * Leftover heaps left out of crop. Same boxes as r17–r26 for delta.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r27-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r27-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r27-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r27-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r27-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r27-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r27-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r27-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r27-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r27-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r27-hole-only', holeImg);
}

// archived boxes from r19/r20/r21/r22/r23/r24/r25/r26 on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92], ['r23densest', 78, 62], ['r24densest', 100, 82], ['r25densest', 100, 82], ['r26densest', 100, 82]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r27-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r27 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r27-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r27-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r27-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r27-tight');
}

{
  const r26 = decodePng(readFileSync(path.join(DIR, '_r26-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r26 fireplace-cavity-tight ===');
  logStats('r26-tight', sampleBuf(r26.width, r26.height, r26.channels, r26.data));
  countGrain(r26.width, r26.height, r26.channels, r26.data, 'r26-tight');
  meanRun(r26.width, r26.height, r26.channels, r26.data, 'r26-tight');
}

{
  const r26 = decodePng(readFileSync(path.join(DIR, '_r26-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r26 fireplace-meat-window ===');
  logStats('r26-meat-window', sampleBuf(r26.width, r26.height, r26.channels, r26.data));
  countGrain(r26.width, r26.height, r26.channels, r26.data, 'r26-meat-window');
  meanRun(r26.width, r26.height, r26.channels, r26.data, 'r26-meat-window');
}

{
  const r26h = decodePng(readFileSync(path.join(DIR, '_r26-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r26 fireplace-hole-only ===');
  logStats('r26-hole-only', sampleBuf(r26h.width, r26h.height, r26h.channels, r26h.data));
  countGrain(r26h.width, r26h.height, r26h.channels, r26h.data, 'r26-hole-only');
  meanRun(r26h.width, r26h.height, r26h.channels, r26h.data, 'r26-hole-only');
}

{
  const r26d = decodePng(readFileSync(path.join(DIR, '_r26-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r26 desk-cavity-tight ===');
  logStats('r26-desk-tight', sampleBuf(r26d.width, r26d.height, r26d.channels, r26d.data));
}

hueSplit('r27-tight', tight);
hueSplit('r27-meat-window', decodePng(readFileSync(path.join(DIR, '_r27-wound-fireplace-meat-window.png'))));
hueSplit('r26-tight', decodePng(readFileSync(path.join(DIR, '_r26-wound-fireplace-cavity-tight.png'))));
hueSplit('r26-meat-window', decodePng(readFileSync(path.join(DIR, '_r26-wound-fireplace-meat-window.png'))));
hueSplit('r26-hole-only', decodePng(readFileSync(path.join(DIR, '_r26-wound-fireplace-hole-only.png'))));
hueSplit('r27-densest-crop', decodePng(readFileSync(path.join(DIR, '_r27-wound-fireplace-hole-only.png'))));
