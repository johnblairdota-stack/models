/**
 * Critic scratch r27 pass 2: 148s densest 80x80 vs r27 hole.
...
for (const b of boxes) {
  ...
  writePng(path.join(DIR, `_r27-bar-${b.tag}.png`), b.w, b.h, crop.ch, crop.buf);
  console.log(`\n=== BAR ${b.tag} (no skip) ===`);
...
{
  const tightBar = decodePng(readFileSync(path.join(DIR, '_r27-bar-148s-hole-tight.png')));
  const z = nearestZoom(tightBar, 4);
  writePng(path.join(DIR, '_r27-zoom-148s-hole-tight.png'), z.w, z.h, z.ch, z.data);
  const best = densest(tightBar, '148s-tight');
  const hole = cropXY(tightBar, best.x, best.y, 80, 80);
  writePng(path.join(DIR, '_r27-bar-148s-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const hz = nearestZoom({ width: 80, height: 80, channels: hole.ch, data: hole.buf }, 4);
  writePng(path.join(DIR, '_r27-zoom-148s-hole-only.png'), hz.w, hz.h, hz.ch, hz.data);
...

{
  ...
  console.log('\n=== r27 densest 80x80 re-sample ===');
  logStats('r27-hole', sampleBuf(r27h.width, r27h.height, r27h.channels, r27h.data));
  hueSplit('r27-hole-only', r27h);
}

{
  const r26h = decodePng(readFileSync(path.join(DIR, '_r26-wound-fireplace-hole-only.png')));
  console.log('\n=== archived r26 densest 80x80 ===');
  logStats('r26-hole', sampleBuf(r26h.width, r26h.height, r26h.channels, r26h.data));
  hueSplit('r26-hole-only', r26h);
}

{
  const r25h = decodePng(readFileSync(path.join(DIR, '_r25-wound-fireplace-hole-only.png')));
  console.log('\n=== archived r25 densest 80x80 ===');
