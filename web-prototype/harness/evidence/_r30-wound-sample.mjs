/**
 * Critic scratch r30: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace densest 80x80 is one #4c3c2c umber island (lip at least
 * r28's 73.9%, not an espresso disc). Voxel grid, not 104x113. Chaise bite is
 * the chaise (velvet/wood of the prop), not a ~0.58 m brown sphere sitting in
 * air. You can still read meat through the fireplace volume (not a black
 * well). Meat must not have gone cream. Flying plates / bodyTint pick are not
 * in the claim. Chaise / settee / table-round / crate plates must not have
 * gone light.
 * Floor-khaki / robot-white / cyc-grey skipped. Near-black KEPT.
 * Leftover heaps left out of crop. Same boxes as r17–r29 for delta.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r30-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r30-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r30-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r30-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r30-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r30-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r30-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r30-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r30-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r30-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r30-hole-only', holeImg);
}

// archived boxes from r19/r20/r21/r22/r23/r24/r25/r26/r27/r28/r29 on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92], ['r23densest', 78, 62], ['r24densest', 100, 82], ['r25densest', 100, 82], ['r26densest', 100, 82], ['r27densest', 79, 62], ['r28densest', 81, 61], ['r29densest', 81, 59]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r30-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r30 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r30-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r30-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r30-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r30-tight');
}

{
  const r29 = decodePng(readFileSync(path.join(DIR, '_r29-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r29 fireplace-cavity-tight ===');
  logStats('r29-tight', sampleBuf(r29.width, r29.height, r29.channels, r29.data));
  countGrain(r29.width, r29.height, r29.channels, r29.data, 'r29-tight');
  meanRun(r29.width, r29.height, r29.channels, r29.data, 'r29-tight');
}

{
  const r29 = decodePng(readFileSync(path.join(DIR, '_r29-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r29 fireplace-meat-window ===');
  logStats('r29-meat-window', sampleBuf(r29.width, r29.height, r29.channels, r29.data));
  countGrain(r29.width, r29.height, r29.channels, r29.data, 'r29-meat-window');
  meanRun(r29.width, r29.height, r29.channels, r29.data, 'r29-meat-window');
}

{
  const r29h = decodePng(readFileSync(path.join(DIR, '_r29-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r29 fireplace-hole-only ===');
  logStats('r29-hole-only', sampleBuf(r29h.width, r29h.height, r29h.channels, r29h.data));
  countGrain(r29h.width, r29h.height, r29h.channels, r29h.data, 'r29-hole-only');
  meanRun(r29h.width, r29h.height, r29h.channels, r29h.data, 'r29-hole-only');
}

{
  const r29d = decodePng(readFileSync(path.join(DIR, '_r29-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r29 desk-cavity-tight ===');
  logStats('r29-desk-tight', sampleBuf(r29d.width, r29d.height, r29d.channels, r29d.data));
}

hueSplit('r30-tight', tight);
hueSplit('r30-meat-window', decodePng(readFileSync(path.join(DIR, '_r30-wound-fireplace-meat-window.png'))));
hueSplit('r29-tight', decodePng(readFileSync(path.join(DIR, '_r29-wound-fireplace-cavity-tight.png'))));
hueSplit('r29-meat-window', decodePng(readFileSync(path.join(DIR, '_r29-wound-fireplace-meat-window.png'))));
hueSplit('r29-hole-only', decodePng(readFileSync(path.join(DIR, '_r29-wound-fireplace-hole-only.png'))));
hueSplit('r30-densest-crop', decodePng(readFileSync(path.join(DIR, '_r30-wound-fireplace-hole-only.png'))));
