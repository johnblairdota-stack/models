/**
 * Critic scratch r35: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace densest recovers toward r32: packed umber island,
 * lip ~80%, not espresso/terra majority. Voxel grid not 104x113.
 * Chaise seat-shaped, not a brown air sphere. You can still read meat
 * through the volume (not a black well). Meat must not have gone cream.
 * Chaise / settee / table-round / crate plates must not have gone light.
 * Floor-khaki / robot-white / cyc-grey skipped. Near-black KEPT.
 * Leftover heaps left out of crop. Same boxes as r17–r34 for delta.
 * Same-coords loop includes r32 densest 90,62 and the prior list plus r34 70,81.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r35-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r35-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r35-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r35-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r35-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r35-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r35-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r35-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r35-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r35-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r35-hole-only', holeImg);
}

// archived boxes from r19–r30 densest coords on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92], ['r23densest', 78, 62], ['r24densest', 100, 82], ['r25densest', 100, 82], ['r26densest', 100, 82], ['r27densest', 79, 62], ['r28densest', 81, 61], ['r29densest', 81, 59], ['r30densest', 100, 100], ['r31densest', 91, 73], ['r32densest', 90, 62], ['r33densest', 89, 63], ['r34densest', 70, 81]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r35-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r35 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r35-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r35-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r35-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r35-tight');
}
...
hueSplit('r35-tight', tight);
hueSplit('r35-meat-window', decodePng(readFileSync(path.join(DIR, '_r35-wound-fireplace-meat-window.png'))));
hueSplit('r35-densest-crop', decodePng(readFileSync(path.join(DIR, '_r35-wound-fireplace-hole-only.png'))));
