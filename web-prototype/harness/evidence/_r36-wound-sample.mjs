/**
 * Critic scratch r36: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace densest 80×80 is a packed umber island (lip toward r32’s
 * 80.7%+, not khaki through a sparse grid). Voxel grid not 104×113.
 ...
 * Leftover heaps left out of crop. Same boxes as r17–r35 for delta.
 * Same-coords loop includes r32 densest 90,62, r35 densest 82,81, r30 100,100,
 * r29 81,59, r28 81,61, r27 79,62, r23 78,62, r24–r26 100,82, r21 82,57,
 * r22 44,92, r19 69,67, r20 38,88.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r36-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r36-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r36-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r36-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r36-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r36-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r36-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r36-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r36-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r36-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r36-hole-only', holeImg);
}

// archived boxes from r19–r30 densest coords on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92], ['r23densest', 78, 62], ['r24densest', 100, 82], ['r25densest', 100, 82], ['r26densest', 100, 82], ['r27densest', 79, 62], ['r28densest', 81, 61], ['r29densest', 81, 59], ['r30densest', 100, 100], ['r31densest', 91, 73], ['r32densest', 90, 62], ['r33densest', 89, 63], ['r34densest', 70, 81], ['r35densest', 82, 81]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r36-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r36 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r36-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r36-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r36-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r36-tight');
}
...
hueSplit('r36-tight', tight);
hueSplit('r36-meat-window', decodePng(readFileSync(path.join(DIR, '_r36-wound-fireplace-meat-window.png'))));
hueSplit('r36-densest-crop', decodePng(readFileSync(path.join(DIR, '_r36-wound-fireplace-hole-only.png'))));
