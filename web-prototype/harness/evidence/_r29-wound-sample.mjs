/**
 * Critic scratch r29: crop leave wounds, sample cavity meat hex, count chip grain.
 * Claim: fireplace densest 80x80 is one #4c3c2c umber island (lip toward 148s
 * 100%), still a voxel grid not a 104x113 espresso slab. Espresso/terra gaps
 * shrink versus r28. You can still read meat through the volume (not a black
 ...
 * Leftover heaps left out of crop. Same boxes as r17–r28 for delta.
...
function runOne(file, box, tag) {
  ...
  const out = path.join(DIR, `_r29-wound-${tag}.png`);
  const hotOut = path.join(DIR, `_r29-hot-${tag}.png`);
...
const tightPath = path.join(DIR, '_r29-wound-fireplace-cavity-tight.png');
const tight = decodePng(readFileSync(tightPath));
const z = nearestZoom(tight, 4);
writePng(path.join(DIR, '_r29-zoom-fireplace-hole.png'), z.w, z.h, z.ch, z.data);
console.log('\nwrote 4x zoom _r29-zoom-fireplace-hole.png');

{
  const meat = decodePng(readFileSync(path.join(DIR, '_r29-wound-fireplace-meat-window.png')));
  const mz = nearestZoom(meat, 5);
  writePng(path.join(DIR, '_r29-zoom-fireplace-meat-window.png'), mz.w, mz.h, mz.ch, mz.data);
  console.log('wrote 5x zoom _r29-zoom-fireplace-meat-window.png');
}
...
// densest 80x80 chromatic lip in tight crop
{
  ...
  writePng(path.join(DIR, '_r29-wound-fireplace-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const holeImg = { width: 80, height: 80, channels: hole.ch, data: hole.buf };
  const hz = nearestZoom(holeImg, 4);
  writePng(path.join(DIR, '_r29-zoom-fireplace-hole-tight.png'), hz.w, hz.h, hz.ch, hz.data);
  ...
  hueSplit('r29-hole-only', holeImg);
}

// archived boxes from r19/r20/r21/r22/r23/r24/r25/r26/r27/r28 on THIS tight crop
{
  for (const [tag, x, y] of [['r19box', 69, 67], ['r20box', 38, 88], ['r21densest', 82, 57], ['r22densest', 44, 92], ['r23densest', 78, 62], ['r24densest', 100, 82], ['r25densest', 100, 82], ['r26densest', 100, 82], ['r27densest', 79, 62], ['r28densest', 81, 61]]) {
    const box = cropXY(tight, x, y, 80, 80);
    writePng(path.join(DIR, `_r29-wound-fireplace-${tag}.png`), 80, 80, box.ch, box.buf);
    console.log(`\n=== fireplace-${tag} (${x},${y}) on r29 tight ===`);
    logStats(tag, sampleBuf(80, 80, box.ch, box.buf));
    countGrain(80, 80, box.ch, box.buf, tag);
    meanRun(80, 80, box.ch, box.buf, tag);
    hueSplit(`r29-${tag}`, { width: 80, height: 80, channels: box.ch, data: box.buf });
  }
}

{
  const cav = decodePng(readFileSync(path.join(DIR, '_r29-wound-fireplace-cavity.png')));
...

{
  ...
  countGrain(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r29-tight');
  meanRun(tightNow.width, tightNow.height, tightNow.channels, tightNow.data, 'r29-tight');
}

{
  const r28 = decodePng(readFileSync(path.join(DIR, '_r28-wound-fireplace-cavity-tight.png')));
  console.log('\n=== DELTA archived r28 fireplace-cavity-tight ===');
  logStats('r28-tight', sampleBuf(r28.width, r28.height, r28.channels, r28.data));
  countGrain(r28.width, r28.height, r28.channels, r28.data, 'r28-tight');
  meanRun(r28.width, r28.height, r28.channels, r28.data, 'r28-tight');
}

{
  const r28 = decodePng(readFileSync(path.join(DIR, '_r28-wound-fireplace-meat-window.png')));
  console.log('\n=== DELTA archived r28 fireplace-meat-window ===');
  logStats('r28-meat-window', sampleBuf(r28.width, r28.height, r28.channels, r28.data));
  countGrain(r28.width, r28.height, r28.channels, r28.data, 'r28-meat-window');
  meanRun(r28.width, r28.height, r28.channels, r28.data, 'r28-meat-window');
}

{
  const r28h = decodePng(readFileSync(path.join(DIR, '_r28-wound-fireplace-hole-only.png')));
  console.log('\n=== DELTA archived r28 fireplace-hole-only ===');
  logStats('r28-hole-only', sampleBuf(r28h.width, r28h.height, r28h.channels, r28h.data));
  countGrain(r28h.width, r28h.height, r28h.channels, r28h.data, 'r28-hole-only');
  meanRun(r28h.width, r28h.height, r28h.channels, r28h.data, 'r28-hole-only');
}

{
  const r28d = decodePng(readFileSync(path.join(DIR, '_r28-wound-desk-cavity-tight.png')));
  console.log('\n=== DELTA archived r28 desk-cavity-tight ===');
  logStats('r28-desk-tight', sampleBuf(r28d.width, r28d.height, r28d.channels, r28d.data));
}

hueSplit('r29-tight', tight);
hueSplit('r29-meat-window', decodePng(readFileSync(path.join(DIR, '_r29-wound-fireplace-meat-window.png'))));
hueSplit('r28-tight', decodePng(readFileSync(path.join(DIR, '_r28-wound-fireplace-cavity-tight.png'))));
hueSplit('r28-meat-window', decodePng(readFileSync(path.join(DIR, '_r28-wound-fireplace-meat-window.png'))));
hueSplit('r28-hole-only', decodePng(readFileSync(path.join(DIR, '_r28-wound-fireplace-hole-only.png'))));
hueSplit('r29-densest-crop', decodePng(readFileSync(path.join(DIR, '_r29-wound-fireplace-hole-only.png'))));
