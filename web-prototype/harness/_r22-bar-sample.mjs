/**
 * Critic scratch r22 pass 2: 148s densest 80x80 + r21 densest coords on r22 tight.
 * Overlay skipped. Same skip rules as wound-sample.
 */
...
const tight = decodePng(readFileSync(path.join(DIR, '_r22-wound-fireplace-cavity-tight.png')));
{
  const box = cropXY(tight, 82, 57, 80, 80);
  writePng(path.join(DIR, '_r22-wound-fireplace-r21densest.png'), 80, 80, box.ch, box.buf);
  console.log('\n=== r22 tight at r21 densest coords (82,57) ===');
  logStats('r21-coords', sampleBuf(80, 80, box.ch, box.buf));
}
...
for (const b of boxes) {
  ...
  writePng(path.join(DIR, `_r22-bar-${b.tag}.png`), b.w, b.h, crop.ch, crop.buf);
  console.log(`\n=== BAR ${b.tag} (no skip) ===`);
...
{
  const tightBar = decodePng(readFileSync(path.join(DIR, '_r22-bar-148s-hole-tight.png')));
  const z = nearestZoom(tightBar, 4);
  writePng(path.join(DIR, '_r22-zoom-148s-hole-tight.png'), z.w, z.h, z.ch, z.data);
  const best = densest(tightBar, '148s-tight');
  const hole = cropXY(tightBar, best.x, best.y, 80, 80);
  writePng(path.join(DIR, '_r22-bar-148s-hole-only.png'), 80, 80, hole.ch, hole.buf);
  const hz = nearestZoom({ width: 80, height: 80, channels: hole.ch, data: hole.buf }, 4);
  writePng(path.join(DIR, '_r22-zoom-148s-hole-only.png'), hz.w, hz.h, hz.ch, hz.data);
...

{
  ...
  console.log('\n=== archived r21 densest 80x80 re-sample ===');
  logStats('r21-hole', sampleBuf(r21h.width, r21h.height, r21h.channels, r21h.data));
}

{
  const r22h = decodePng(readFileSync(path.join(DIR, '_r22-wound-fireplace-hole-only.png')));
  console.log('\n=== r22 densest 80x80 re-sample ===');
  logStats('r22-hole', sampleBuf(r22h.width, r22h.height, r22h.channels, r22h.data));
}

// hue split: share of keep in terracotta 0.13-0.18 vs dusty 0.28-0.35 vs lip-umber 0.20-0.28
...
hueSplit('r21-tight', decodePng(readFileSync(path.join(DIR, '_r21-wound-fireplace-cavity-tight.png'))));
hueSplit('r21-meat-window', decodePng(readFileSync(path.join(DIR, '_r21-wound-fireplace-meat-window.png'))));
hueSplit('r21-hole-only', decodePng(readFileSync(path.join(DIR, '_r21-wound-fireplace-hole-only.png'))));
hueSplit('148s-tight', decodePng(readFileSync(path.join(DIR, '_r22-bar-148s-hole-tight.png'))));
hueSplit('148s-hole-only', decodePng(readFileSync(path.join(DIR, '_r22-bar-148s-hole-only.png'))));
hueSplit('r22-at-r21-coords', decodePng(readFileSync(path.join(DIR, '_r22-wound-fireplace-r21densest.png'))));
