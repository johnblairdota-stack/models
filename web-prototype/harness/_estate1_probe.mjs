 ...
 *
 *   node harness/_estate1_probe.mjs --out progress --anchor gallery.mid
 *   node harness/_estate1_probe.mjs --arms "off:,port:estate=port"
 */
...
const ARMS = opt('arms', 'off:|port:estate=port').split('|');
...
for (const spec of ARMS) {
  ...
  const res = await page.evaluate(async ({ anchor, yaws, shotYaw }) => {
    ...
    snap(shotYaw);
    for (let k = 0; k < 4; k++) { await new Promise((r) => setTimeout(r, 150)); snap(shotYaw); }
    e.endPerf();
    ...
    const gal = room.spaces.find((s) => s.id === 'gallery');
    let galMeshes = 0;
    ...
    return {
      ...
      estate: room.estate ? { on: room.estate.on, order: room.estate.order,
        surfaces: room.estate.surfaces, plans: Object.keys(room.estate.plans ?? {}) } : null,
      rig: e.__estateRig ?? null,
      galMeshes,
      programs: e.renderer.info.programs?.length ?? 0,
    };
  }, { anchor: ANCHOR, yaws: YAWS, shotYaw: SHOT_YAW });

  const file = path.join(OUT, `estate1-${ANCHOR.replace('.', '-')}-${name}.png`);
  ...
  console.log(`  gallery space meshes ${res.galMeshes}   estate ${JSON.stringify(res.estate)}`);
  if (res.rig) console.log(`  rig ${JSON.stringify(res.rig)}`);
}
