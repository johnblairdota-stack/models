...
async function runCombo(id, buildBallots, opts = {}) {
  const CODE = 'n7' + Math.random().toString(36).slice(2, 5);
...
await runCombo('N7-unanimous', (ids) => ids.map(() => ({ runner: ids[0], guide: ids[1] })));

await runCombo('N7-near-tie-plurality', (ids) => [
  { runner: ids[0], guide: ids[6] },
  { runner: ids[0], guide: ids[6] },
  { runner: ids[0], guide: ids[6] },
  { runner: ids[1], guide: ids[6] },
  { runner: ids[1], guide: ids[6] },
  { runner: ids[2], guide: ids[6] },
  { runner: ids[3], guide: ids[6] },
], { note: 'runner 3-2-1-1 plurality; guide clear Gus' });

await runCombo('N7-multiway-split', (ids) => ids.map((_, i) => ({
  runner: ids[i], guide: ids[(i + 1) % N],
})), { note: '7-way runner split → seeded' });

await runCombo('N7-self-pick-void', (ids) => [
...
await runCombo('N7-late-ballot', (ids) => [
...
await runCombo('N7-partial-ballots', (ids) => [
  { runner: ids[0], guide: ids[1] },
...
await runCombo('N7-guide-runner-conflict', (ids) => [
  { runner: ids[0], guide: ids[1] },
...
{
  const CODE = 'n7e' + Math.random().toString(36).slice(2, 4);
  const { tv, phones } = await seatN(CODE, N);
  send(tv, { t: 'episode', opts: {} });
  await sleep(350);
  const last = tv.msgs.filter((m) => m.t === 'state').at(-1);
  const row = { id: 'N7-empty-noop', N, pair: last?.frame?.pair || null, phase: last?.frame?.phase, pass: !last?.frame?.pair?.runner, note: 'empty+unused must not invent' };
  results.push(row); console.log(JSON.stringify(row)); closeAll(tv, phones);
}

{
  const CODE = 'n7f' + Math.random().toString(36).slice(2, 4);
  const { tv, phones, ids } = await seatN(CODE, N);
  ...
  const last = tv.msgs.filter((m) => m.t === 'state').at(-1);
  const row = {
    id: 'N7-force-spare-with-ballots', N,
...
await writeFile('progress/overnight-vote/loop3-n7-wire.json', JSON.stringify({ at: new Date().toISOString(), head, N, results }, null, 2));
const fails = results.filter((r) => !r.pass).length;
console.log('WIRE_SUMMARY_N7', { pass: results.length - fails, fail: fails, total: results.length, head });
