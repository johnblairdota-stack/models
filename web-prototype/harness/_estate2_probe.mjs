...
const ANCHOR = opt('anchor', 'study_w.north');
const SPACE = opt('space', 'study_w');
const SHOT_YAW = +opt('yaw', Math.PI / 2);
const YAWS = +opt('yaws', 8);
const ARMS = opt('arms', 'nostudy:estate=port,nostudy|study:estate=port').split('|');
...
for (const spec of ARMS) {
  ...
  const res = await page.evaluate(async ({ anchor, yaws, shotYaw, spaceId }) => {
  ...
  }, { anchor: ANCHOR, yaws: YAWS, shotYaw: SHOT_YAW, spaceId: SPACE });

  const file = path.join(OUT, `estate2-${ANCHOR.replace('.', '-')}-${name}.png`);
