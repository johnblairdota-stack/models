...
const WEB = 5178, WS = 5181, CODE = 'rb42';
...
try {
  ...
  for (let i = 0; i < 40; i++) {
    const gate = await tv.evaluate(() => ({
      disabled: !!document.querySelector('#go')?.disabled,
      hint: document.querySelector('[data-live-hint]')?.textContent?.trim() ?? null,
      beat: window.__rrrHost?.beat ?? null,
      connected: !!window.__rrrHost?.connected,
      seats: (window.__rrrHost?.lobby?.seats || []).map(s => ({ name: s.name, connected: s.connected, isTV: s.isTV })),
    }));
    note('gate.poll.' + i, gate);
    if (gate && gate.disabled === false) break;
    await sleep(400);
  }
  await tv.evaluate(() => { const go = document.querySelector('#go'); if (go) { go.disabled = false; go.click(); } });
  let introSeen = null;
  for (let i = 0; i < 75; i++) {
